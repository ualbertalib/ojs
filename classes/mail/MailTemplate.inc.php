...
**/
		...
		if ($journal && !Request::getUserVar('continued')) {
			...
			if( Request::getUserVar('articleId') ){
				$customArticleId = Request::getUserVar('articleId');
			}
				if (! isset($customArticleId) ){
					$this->setSubject('[' . $journal->getLocalizedSetting('initials') . '] ' . $this->getSubject() . ' [Article ID:{$articleId}]' );
				}else{
					$this->setSubject('[' . $journal->getLocalizedSetting('initials') . '] ' . $this->getSubject() . " [Article ID:{$customArticleId}]" );
				}
		}
